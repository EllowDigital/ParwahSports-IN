...
export default function ParticipationsManager() {
  ...
  const { data: participations, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("participations")
        .select(
          `*, students:student_id(name, email), competitions:competition_id(name, event_date)`,
        )
  ...
  const filteredParticipations =
    participations?.filter(
      (p) =>
        (p.students?.name || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
        (p.competitions?.name || "").toLowerCase().includes(searchTerm.toLowerCase()),
    ) || [];
    ...
    <AdminLayout>
      <div className="space-y-6">
        <PageHeader
          ...
          description="Manage student participations in competitions"
          searchPlaceholder="Search by student or competition..."
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                    {searchTerm ? "No matching participations" : "No participations found"}
                  </TableCell>
                </TableRow>
              ) : (
                ...
                <SelectContent>
                  {competitions?.map((c) => (
                    <SelectItem key={c.id} value={c.id}>
                      {c.name} ({format(new Date(c.event_date), "MMM d, yyyy")})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
      ...
      <ConfirmDialog
        ...
        description="Are you sure you want to delete this participation record?"
