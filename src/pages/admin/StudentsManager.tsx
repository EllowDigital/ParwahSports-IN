...
export default function StudentsManager() {
      ...
      if (data.create_account && data.password) {
        const { data: created, error: createErr } =
          await supabase.functions.invoke<CreateUserResponse>("admin-create-student-user", {
            body: { email: data.email, password: data.password, full_name: data.name },
          });
  ...
  const activateAndLinkMutation = useMutation({
    mutationFn: async ({ student, password }: { student: Student; password: string }) => {
      ...
      const { data: created, error: createErr } =
        await supabase.functions.invoke<CreateUserResponse>("admin-create-student-user", {
          body: { email: student.email, password, full_name: student.name },
        });
      if (createErr) throw createErr;
    ...
    onSuccess: ({ user_id }) => {
      queryClient.invalidateQueries({ queryKey: ["admin-students"] });
      toast({ title: "Student activated + linked", description: `Auth user created: ${user_id}` });
      setLinkPassword("");
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <PageHeader
          title="Students"
          description={`${students?.length || 0} students`}
