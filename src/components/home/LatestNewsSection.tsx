        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          {news.map((item, index) => (
            <ScrollReveal key={item.id} delay={index * 80}>
              <Card
                className={`group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 h-full border-border hover:border-primary/20 ${
                  index === 0 ? "sm:col-span-2" : ""
                }`}
              >
                <CardContent
                  className={`p-5 lg:p-6 h-full flex flex-col ${index === 0 ? "sm:flex-row sm:gap-6" : ""}`}
                >
                  {index === 0 && (
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <p
                      className={`text-muted-foreground line-clamp-3 flex-1 ${
