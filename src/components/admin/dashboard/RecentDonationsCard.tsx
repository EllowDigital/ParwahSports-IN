...
import { IndianRupee, ArrowRight, LayoutGrid, List } from "lucide-react";
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {donations.map((d) => (
              <div
                key={d.id}
                className="flex items-center gap-3 p-3 rounded-xl border border-border/50 hover:border-primary/30 transition-all"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground truncate">{d.donor_email}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold text-foreground">₹{Number(d.amount).toLocaleString("en-IN")}</p>
                  <p className="text-[10px] text-muted-foreground">
                    {formatDistanceToNow(new Date(d.created_at), { addSuffix: true })}
                  </p>
                </div>
                <Badge variant="outline" className={`text-[9px] shrink-0 ${statusColor[d.payment_status || "pending"] || ""}`}>
