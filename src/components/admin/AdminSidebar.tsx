...
import {
  LayoutDashboard, Image, Calendar, Users, UserCheck, Settings, LogOut, Home, Menu, X, Heart,
  CreditCard, Crown, Newspaper, FileText, Bell, Trophy, GraduationCap, Award, Medal,
} from "lucide-react";
...
export function AdminSidebar() {
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="p-3 border-t border-border space-y-2">
        <Link to="/" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors">
        ...
        {user && <div className="px-3 py-2 text-xs text-muted-foreground truncate">{user.email}</div>}
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      {isMobileMenuOpen && <div className="lg:hidden fixed inset-0 z-40 bg-background/80 backdrop-blur-sm" onClick={() => setIsMobileMenuOpen(false)} />}
      <aside className={cn("lg:hidden fixed top-14 left-0 bottom-0 z-50 w-64 bg-card border-r border-border flex flex-col transition-transform duration-300", isMobileMenuOpen ? "translate-x-0" : "-translate-x-full")}>
        <SidebarContent />
      </aside>
